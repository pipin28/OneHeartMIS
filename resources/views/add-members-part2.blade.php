...
<head>
    ...
    <title>Add Members - Member Details | {{ $appBrandName }}</title>
...
<body class="has-shell" data-draft="{{ $isDraft ? '1' : '0' }}">
        ...
        <main class="dashboard">
            <section class="wrap">
                ...
                <div class="form-actions" style="justify-content: flex-start; margin-bottom: 8px;">
                    <a href="{{ $isDraft ? route('add-members.draft.staff') : route('add-members.edit', ['part1' => $part1Id]) }}" class="button" style="box-shadow: none;">{{ $isDraft ? 'Back to Staff Info' : 'Back to Member Enrollment' }}</a>
                </div>
                ...
                <form method="POST" action="{{ $isDraft ? '#' : route('add-members.part2.store', $part1Id) }}" class="form-grid" id="memberDetailsForm">
                    ...
                    <div>
                        ...
                        <input type="text" id="reference_number" name="reference_number" value="{{ old('reference_number', $part1->reference_number ?? '') }}" placeholder="Reference number" required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="date" id="date_of_birth" name="date_of_birth" value="{{ old('date_of_birth', $part2->date_of_birth ?? '') }}" max="{{ now()->subYears(60)->toDateString() }}" required>
                    </div>
                    <div>
    ...
    <div class="status-modal {{ $errors->any() ? 'is-visible' : '' }}" id="memberAgeErrorModal" data-message="{{ $errors->first() }}">
        <div class="status-card">
    ...
    <script>
        const memberAgeMessage = "The member age must be not below 60 years old.";
        const showMemberAgeModal = (message = memberAgeMessage) => {
            const modal = document.getElementById('memberAgeErrorModal');
            if (!modal) {
                alert(message);
                return;
            }
            const body = modal.querySelector('.status-body');
        ...
        (() => {
            ...
            const seedDobPickerYear = () => {
                if (!dob || dob.value || !dob.max) return;
                dob.value = dob.max;
                dobPickerSeeded = true;
                if (age) age.value = computeAge(dob.value);
            };
            dob?.addEventListener('focus', seedDobPickerYear);
            dob?.addEventListener('pointerdown', seedDobPickerYear);
            dob?.addEventListener('change', () => {
                dobPickerSeeded = false;
            ...
            form?.addEventListener('submit', (e) => {
                const rawValue = age?.value || computeAge(dob?.value);
                const value = Number(rawValue);
                if (rawValue !== '' && (!Number.isFinite(value) || value < 60)) {
                    e.preventDefault();
                    showMemberAgeModal();
                }
            });
        ...
        (() => {
            ...
            const memberAgeIsValid = () => {
                const rawValue = form?.querySelector('[name="age"]')?.value || '';
                const value = Number(rawValue);
                if (rawValue !== '' && (!Number.isFinite(value) || value < 60)) {
                    showMemberAgeModal();
                    return false;
